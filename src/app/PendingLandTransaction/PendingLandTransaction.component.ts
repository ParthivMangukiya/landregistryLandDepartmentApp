...
import { ApprovePendingLandTransactionService } from '../ApprovePendingLandTransaction/ApprovePendingLandTransaction.service';
...
@Component({
  ...
  providers: [PendingLandTransactionService,ApprovePendingLandTransactionService]
})
export class PendingLandTransactionComponent implements OnInit {

  myForm: FormGroup;
  myApproveForm: FormGroup;
  ...
  constructor(private appComponent:AppComponent,private serviceApprovePendingLandTransaction: ApprovePendingLandTransactionService,public servicePendingLandTransaction: PendingLandTransactionService, fb: FormBuilder) {
  ...
  setHash(hash : String){
        this.newDocumentSignature.setValue(hash);
  }
  ...
  resetApproveForm(id: any): void {
    this.myApproveForm.setValue({
      'newDocumentSignature': null,
      'pendingLandTransaction': id
    });
  }

  enableApprove(state1:any): boolean{
    return state1=="requestedLandDepartment";
  }

  addTransaction(form: any): Promise<any> {
    this.Transaction = {
      $class: 'org.svnit.comps.ApprovePendingLandTransaction',
      'pendingLandTransaction': 'resource:org.svnit.comps.PendingLandTransaction#'+this.pendingLandTransaction.value,
      'newDocumentSignature': this.newDocumentSignature.value,
      'approve': true,
      'transactionId': null,
      'timestamp': null
    ...
    return this.serviceApprovePendingLandTransaction.addTransaction(this.Transaction)
    .toPromise()
    .then(() => {
      this.errorMessage = null;
      this.myApproveForm.setValue({
  ...
  disapproveTransaction(id: any): Promise<any> {
    this.Transaction = {
      $class: 'org.svnit.comps.ApprovePendingLandTransaction',
